...
#ifndef _SIMPLEXNOISE3_GLSL_
#define _SIMPLEXNOISE3_GLSL_
											...
											out vec4 v1234_z )
		{
			...
			Pi = floor( P + dot( P, vec3( SKEWFACTOR, SKEWFACTOR, SKEWFACTOR) ) );
			vec3 x0 = P - Pi + dot(Pi, vec3( UNSKEWFACTOR, UNSKEWFACTOR, UNSKEWFACTOR ) );
		...
		float SimplexBillowed(vec3 p, int octaves, vec3 offset, float frequency, float amplitude, float lacunarity, float persistence)
		{
			float sum = 0;

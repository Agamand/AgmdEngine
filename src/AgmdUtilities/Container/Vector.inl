...
template <typename T, template <typename> class A> inline void Vector<T, A>::pop_back()
{
    m_storage[m_size] = T::~T();
    m_size && m_size--;
}

template <typename T, template <typename> class A> inline void Vector<T, A>::resize(a_uint32 size)
{
    if(size > m_capacity)
...
template <typename T, template <typename> class A> inline void Vector<T, A>::resize(a_uint32 size,const T val)
{
    if(size > m_capacity)
        resize_capacity(size+DEFAULT_CAPACITY);
    if(size > m_size)
...
template <typename T, template <typename> class A> inline void Vector<T, A>::resize_capacity(a_uint32 capacity)
{
...
template <typename T, template <typename> class A> inline a_uint32 Vector<T, A>::size() const
{
    return m_size;
}

template <typename T, template <typename> class A> inline void Vector<T, A>::clear()
{
    m_size = 0;
}

template <typename T, template <typename> class A> inline a_uint32 Vector<T, A>::capacity() const
{
    return m_capacity;
}


template <typename T, template <typename> class A> inline T& Vector<T, A>::operator[](a_uint32 i)
{
    return *(m_storage+i);
}
template <typename T, template <typename> class A> inline const T& Vector<T, A>::operator[](a_uint32 i) const
{
    return *(m_storage+i);
}
/*

...
#include <math.h>
#include "Planet.h"
PlanetAtmosphereNode::PlanetAtmosphereNode(Planet* controller) : MeshNode(GeometryFactory::createSphere(1,100,100,(float)M_PI*2)),m_controller(controller)
{
...
void PlanetAtmosphereNode::render( TRenderPass pass ) const
{
	...
	if(m_cam_dist > m_controller->m_size*OUTER_INNER_OFFSET)
	{
	...
	m_skyProgram[use].SetParameter("v3CameraPos",m_cam_position);
	...
	m_skyProgram[use].SetParameter("fCameraHeight",length(m_cam_position));
	m_skyProgram[use].SetParameter("fCameraHeight2",length(m_cam_position)*length(m_cam_position));
	...
	m_groundProgram[use].SetParameter("v3CameraPos",m_cam_position);
	...
	m_groundProgram[use].SetParameter("fCameraHeight",length(m_cam_position));
	m_groundProgram[use].SetParameter("fCameraHeight2",length(m_cam_position)*length(m_cam_position));
...
void PlanetAtmosphereNode::findVisible( Camera*cam, RenderQueue& display, a_vector<LightNode*>& light )
{
	...
	m_cam_position = m_cam_position-vec3(my);
	m_cam_dist = length(m_cam_position);
